...
This release introduces a **color theme** for dashboards — consistent chart, budget, and category colors across every dashboard. Demo dashboards you **haven't customized** pick it up automatically when you apply the update. This includes any dashboard Finzytrack upgraded to a new internal format during the update: **that automatic upgrade doesn't count as your edit**, so those dashboards still get the new colors on their own. Only a demo dashboard **you deliberately changed** is left as-is (never overwritten), so it won't switch to the improved, fully-themed version by itself.

A demo dashboard you've customized has two ways to get the new theme, with a trade-off between them:

- **Keep your version and theme it yourself.** To preserve your edits, leave your dashboard in place and change its color fields to use theme tokens like `{{theme.brand}}` instead of fixed color values. This adopts the theme *without* discarding anything you changed. See [Dashboard Colors & Themes](/reference/dashboard-themes/) for the available tokens. (This is also how you theme a dashboard you built yourself — it keeps whatever colors you gave it until you swap in tokens.)
- **Replace it with the shipped version.** The simplest way to get the current, fully-themed dashboard — but it **replaces your customizations** with the shipped copy. Finzytrack saves a timestamped backup of your version first, so you can refer to it or roll back, but it does **not** merge your edits into the new version; you'd re-apply any changes you want to keep by hand. Steps are the same as [Restoring an original demo](#restoring-an-original-demo): open the shipped copy from the repository's [`backend/resources/seed_config/recipes/dashboards/`](https://github.com/sagarbehere/finzytrack/tree/main/backend/resources/seed_config/recipes/dashboards), paste it into **Settings → Dashboards**, and **Save**.
